...
def main():
    ...
    for cobol, stem, program_id, program in program_infos:
        ...
        # The physical source map. Every other artifact describes the program;
        # this one *is* the program, line for line, and the source-address
        # capability reads it directly instead of retrieving. It was previously
        # generated by hand for one program, so line lookups worked there and
        # silently failed everywhere else.
        run_cmd([
            python, str(PIPELINE_SCRIPTS_DIR / "build_source_lines.py"),
            "--package", str(cobol.parent.parent),
            "--program", program,
